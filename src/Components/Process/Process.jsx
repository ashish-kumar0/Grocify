...
const Process = () => {
  ...
  const renderSteps = steps.map(items=>{
    return(
        <div className= {`flex-1 basis-[300px] ${items.id %2 ===0 ? 'md:-mt-100' : ''}`}>
            <span className='flex justify-center items-center w-18 h-18 mx-auto rounded-full text-8xl bg-zinc-800 outline-[3px] outline-offset-7 outline-zinc-800 text-white outline-dashed'>{items.number}</span>
