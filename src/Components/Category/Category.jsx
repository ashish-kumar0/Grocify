    ...
    return (
      ...
      <div key={card.id} className="flex-1 basis-[300px]">
        ...
        <div className="bg-zinc-100 pt-17 p-8 rounded-xl">
          ...
          <p className="text-zinc-600 mt-2 mb-9">{card.description}</p>
          <Link to={card.path}
            className="bg-gradient-to-b from-orange-400 to-orange-500 text-white px-8 py-3 
             rounded-lg md:text-lg text-md hover:scale-105 hover:to-orange-600 transition-all duration-300 
             cursor-pointer">
            See all
          </Link>
        </div>
  ...
  return (
    <section>
      <div className="max-w-[1400px] mx-auto px-10 py-20">
        ...
        {/* Category Cards */}
        <div className="flex flex-wrap gap-10 md:mt-15 mt-10">{renderCard}</div>
      </div>
    </section>
  );
};
...
const category = [
...

  {
    ...
    title: "Dairy & Eggs",
    description:
      "Wholsome dairy products and free-range eggs. From creamy milk and yogurt to artisanal cheeses",
    image: DairyCat,
    path: '/dairy',
  },
